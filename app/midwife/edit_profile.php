...
if (isset($_POST['submit_info'])) {
    ...
    //Query for updating 
    $stmt = $conn->prepare("UPDATE midwife SET name=?, province=?, city=?, barangay=?, contactno=?, email=?, updationDate=? WHERE id=?");
    $stmt->bind_param("sssssssi", $docname, $province, $city, $barangay, $contact, $email, $updationDate, $midwife_id);
...
if (isset($_POST['submit_pass'])) {
    ...
    if ($new_pass !== $confirm_pass) {
        ...
        $stmt = $conn->prepare("SELECT password FROM midwife WHERE id = ?");
        ...
        if ($user && password_verify($current_pass, $user['password'])) {
            ...
            $update_stmt = $conn->prepare("UPDATE midwife SET password=?, updationDate=? WHERE id=?");
            $update_stmt->bind_param("ssi", $hashed_new_pass, $updationDate, $midwife_id);
    ...
    <div class="main-content">
        <div class="wrap-content container" id="container">
            <section id="page-title">
                <div class="row">
                    <div>
                        <h1 class="main-title">Edit Midwife Informations</h1>
                    </div>
                </div>
            </section>
                                ...
                                <div class="card shadow-lg">
                                    <div class="card-body">
                                        ...
                                        <form role="form" name="update_information" method="post">
                                            ...
                                            <?php
                                            $ret = mysqli_query($conn, "SELECT name, province, city, barangay, contactno, email FROM midwife WHERE id = '$midwife_id'");
                                            $midwife = mysqli_fetch_assoc($ret);
                                            ?>

                                            <div class="form-group mb-3">
                                                <label for="docname">Doctor name</label>
                                                <input type="text" name="docname" class="form-control" placeholder="Surname, First Name MI." value="<?php echo htmlentities($midwife['name']); ?>">
                                            </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select id="city" class="form-control">
                                                    <option value="<?= htmlspecialchars($midwife['city']) ?>" selected>
                                                        <?= htmlspecialchars($midwife['city']) ?>
                                                    </option>
                                                </select>
                                            </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select id="barangay" class="form-control">
                                                    <option value="<?= htmlspecialchars($midwife['barangay']) ?>" selected>
                                                        <?= htmlspecialchars($midwife['barangay']) ?>
                                                    </option>
                                                </select>
                                            </div>

                                            <!-- Hidden Fields -->
                                            <input type="hidden" name="province" id="province_name" value="<?= htmlspecialchars($midwife['province']) ?>">
                                            <input type="hidden" name="city" id="city_name" value="<?= htmlspecialchars($midwife['city']) ?>">
                                            <input type="hidden" name="barangay" id="barangay_name" value="<?= htmlspecialchars($midwife['barangay']) ?>">

                                            <div class="form-group mb-3">
                                                <label for="contactno">Contact no.</label>
                                                <input type="text" name="doccontact" class="form-control" placeholder="Enter Doctor Contact No." value="<?php echo htmlentities($midwife['contactno']); ?>">
                                            </div>

                                            <div class="form-group mb-3">
                                                <label for="email">Email</label>
                                                <input type="email" id="docemail" name="docemail" class="form-control" placeholder="Enter Doctor Email" value="<?php echo htmlentities($midwife['email']); ?>">
                                            </div>
